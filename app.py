...
Tip: to add of silence (1.4 seconds) into your text just use "###" or "[pause]".
    ...
    options = [
        ...
        '--ebook', '--ebooks_dir', '--language', '--enable_translation', '--target_language', '--voice', '--device', '--tts_engine',
    ...
    headless_group.add_argument(options[6], type=str, default=default_language_code, help=f'''Language of the e-book (source language if translation is enabled). Default language is set
    in ./lib/lang.py sed as default if not present. All compatible language codes are in ./lib/lang.py''')
    headless_optional_group = parser.add_argument_group('optional parameters')
    headless_optional_group.add_argument(options[7], action='store_true', help='''(Optional) Enable translation from source language to target language before TTS.''')
    headless_optional_group.add_argument(options[8], type=str, default=None, help='''(Optional) Target language for TTS when translation is enabled.
    Must be used with --enable_translation. All compatible language codes are in ./lib/lang.py''')
    headless_optional_group.add_argument(options[9], type=str, default=None, help='''(Optional) Path to the voice cloning file for TTS engine.
    Uses the default voice if not present.''')
    headless_optional_group.add_argument(options[10], type=str, default=default_device, choices=device_list, help=f'''(Optional) Pprocessor unit type for the conversion.
    Default is set in ./lib/conf.py if not present. Fall back to CPU if GPU not available.''')
    headless_optional_group.add_argument(options[11], type=str, default=None, choices=tts_engine_list_keys+tts_engine_list_values, help=f'''(Optional) Preferred TTS engine (available are: {tts_engine_list_keys+tts_engine_list_values}.
    Default depends on the selected language. The tts engine should be compatible with the chosen language''')
    headless_optional_group.add_argument(options[12], type=str, default=None, help=f'''(Optional) Path to the custom model zip file cntaining mandatory model files.
    Please refer to ./lib/models.py''')
    headless_optional_group.add_argument(options[13], type=str, default=default_fine_tuned, help='''(Optional) Fine tuned model path. Default is builtin model.''')
    headless_optional_group.add_argument(options[14], type=str, default=default_output_format, help=f'''(Optional) Output audio format. Default is set in ./lib/conf.py''')
    headless_optional_group.add_argument(options[15], type=float, default=None, help=f"""(xtts only, optional) Temperature for the model.
    Default to config.json model. Higher temperatures lead to more creative outputs.""")
    headless_optional_group.add_argument(options[16], type=float, default=None, help=f"""(xtts only, optional) A length penalty applied to the autoregressive decoder.
    Default to config.json model. Not applied to custom models.""")
    headless_optional_group.add_argument(options[17], type=int, default=None, help=f"""(xtts only, optional) Controls how many alternative sequences the model explores. Must be equal or greater than length penalty.
    Default to config.json model.""")
    headless_optional_group.add_argument(options[18], type=float, default=None, help=f"""(xtts only, optional) A penalty that prevents the autoregressive decoder from repeating itself.
    Default to config.json model.""")
    headless_optional_group.add_argument(options[19], type=int, default=None, help=f"""(xtts only, optional) Top-k sampling.
    Lower values mean more likely outputs and increased audio generation speed.
    Default to config.json model.""")
    headless_optional_group.add_argument(options[20], type=float, default=None, help=f"""(xtts only, optional) Top-p sampling.
    Lower values mean more likely outputs and increased audio generation speed. Default to config.json model.""")
    headless_optional_group.add_argument(options[21], type=float, default=None, help=f"""(xtts only, optional) Speed factor for the speech generation.
    Default to config.json model.""")
    ...
    Default to config.json model.""")
    headless_optional_group.add_argument(options[23], type=float, default=None, help=f"""(bark only, optional) Text Temperature for the model.
    Default to {default_engine_settings[TTS_ENGINES['BARK']]['text_temp']}. Higher temperatures lead to more creative outputs.""")
    headless_optional_group.add_argument(options[24], type=float, default=None, help=f"""(bark only, optional) Waveform Temperature for the model.
    ...
    headless_optional_group.add_argument(options[26], action='store_true', help='''(Optional) Force restart from beginning, ignoring any existing checkpoint.''')
    headless_optional_group.add_argument(options[27], action='version', version=f'ebook2audiobook version {prog_version}', help='''Show the version of the script and exit''')
    headless_optional_group.add_argument(options[28], action='store_true', help=argparse.SUPPRESS)
